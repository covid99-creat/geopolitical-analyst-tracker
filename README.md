...
- השרת מושך RSS/Atom מכל מקור.
- חלון סריקה ברירת מחדל: 180 ימים אחורה (`LOOKBACK_DAYS`).
- מסנן אייטמים עם ניסוח תחזיתי מורחב (למשל `will`, `likely`, `forecast`, `could`, `scenario`, `צפוי`).
- כולל גם זיהוי הקשר גיאו-פוליטי כדי להגדיל Recall (ולא לפספס תחזיות מנוסחות בעדינות).
